...
class EditPage extends StatefulWidget {
  final Map<String, dynamic> item; // รับข้อมูลไอเทมเดิมมา
...
class _EditPageState extends State<EditPage> {
  ...
  XFile? _imageFile; // รูปภาพใหม่ (ถ้ามีการเลือก)
  String _existingImageUrl = ''; // เก็บ URL รูปภาพเดิม
  ...
  // ดึงข้อมูลเก่ามากระจายลงช่องต่างๆ
  void _populateExistingData() {
    ...
    _existingImageUrl = widget.item['image'] ?? '';

    // แยกวันที่หมดอายุ (DD-MM-YYYY)
    String expDate = widget.item['expiryDate'] ?? '';
    if (expDate.contains('-')) {
      var parts = expDate.split('-');
      if (parts.length == 3) {
        _expDD.text = parts[0];
        _expMM.text = parts[1];
        _expYYYY.text = parts[2];
      }
    }

    // แยกวันที่ผลิต (DD-MM-YYYY)
    String mfgDate = widget.item['manufacturingDate'] ?? '';
    if (mfgDate.contains('-')) {
      var parts = mfgDate.split('-');
      if (parts.length == 3) {
        _mfgDD.text = parts[0];
        _mfgMM.text = parts[1];
        _mfgYYYY.text = parts[2];
      }
    }
  }
  ...
  Future<String?> _uploadToCloudinary(XFile file) async {
    ...
    if (response.statusCode == 200) {
      ...
      String exactError = 'Upload Failed: ${response.statusCode}';
  ...
  void _submitForm() async {
    ...
    // ถ้ารูปใหม่เป็น null และรูปเก่าก็ไม่มี (ป้องกันบัค)
    if (_imageFile == null && _existingImageUrl.isEmpty) {
      _imageError = 'Please upload a food picture';
      hasError = true;
    }

    if (!_isValidRealDate(_expDD.text, _expMM.text, _expYYYY.text)) {
      _expDateError = 'Invalid expire date';
      hasError = true;
    }
    ...
    if (hasMfgInput) {
      if (!_isValidRealDate(_mfgDD.text, _mfgMM.text, _mfgYYYY.text)) {
        _mfgDateError = 'Invalid manufacturing date';
        hasError = true;
      }
    }
    ...
    try {
      // 1. จัดการรูปภาพ (ถ้ามีเลือกรูปใหม่ให้อัปโหลด ถ้าไม่ ให้ใช้ของเดิม)
      ...
      // 2. อัปเดตข้อมูลลง Firestore ผ่าน Document ID
      ...
      if (mounted) {
        // เมื่อ Edit เสร็จ ให้เด้งกลับไปหน้าแรก (Homepage) ทันที เพื่อให้ข้อมูลอัปเดตใหม่
  ...
  Widget _buildImagePicker() {
    return GestureDetector(
      ...
      child: Container(
        ...
        // ลอจิกแสดงรูป: ถ้ารูปใหม่ถูกเลือก ให้โชว์รูปใหม่ ถ้ายังไม่เลือกให้โชว์รูปจาก URL เดิม
